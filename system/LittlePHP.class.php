...
class LITTLE
{
	...
	*/
    public static function getLITTLEDirectory() {
        return realpath(dirname(__FILE__)."/../../.");
    }
    
    function getPluginsDirectory() {
        // must be set by configuration
        return LITTLE::getLITTLEDirectory()."/system/plugins";
    }
    
    /**
    * If you need static properties, you can use this method
    * to simulate them. Eg. in your method(s)
    * do this: $myVar = &LITTLE::getStaticProperty('myVar');
    ...
    */
     public static function __getVar($nom_var) {
        ...
        $config = LITTLE::getStaticProperty('LITTLE','config');
        
        // if configuration data is not set
        if (!isset($config)) {
		     //print_r("<br>No esta Seteada __getVar	 jj ->".(dirname(dirname(__FILE__)))."<br>");
            // load the configuration data
            // filename = <LITTLE-APP-dir>/config/system.conf.data  getLITTLEDirectory
